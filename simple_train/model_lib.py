...
def unstack_batch(tensor_dict, unpad_groundtruth_tensors=True):
    ...
    if unpad_groundtruth_tensors:
        if fields.InputDataFields.num_groundtruth_boxes not in unbatched_tensor_dict:
            raise ValueError('`num_groundtruth_boxes` not found in tensor_dict. Keys available: {}'.
...
def create_model_fn(detection_model_fn, configs, hparams):
    ...
    def model_fn(features, labels, mode, params=None):
        ...
        # Make sure to set the Keras learning phase. True during training, False for inference.
        tf.keras.backend.set_learning_phase(is_training)
        # init model class
        detection_model = detection_model_fn(model_config=model_config, is_training=is_training, add_summaries=True)

        # unstack label--按batch维 解压成 list
        if mode == tf.estimator.ModeKeys.TRAIN:
            labels = unstack_batch(labels, unpad_groundtruth_tensors=train_config.unpad_groundtruth_tensors)
        elif mode == tf.estimator.ModeKeys.EVAL:
            # For evaling on train data, it is necessary to check whether groundtruth must be unpadded.
        ...
        # 2.postprocess--评估和预测阶段
        if mode in (tf.estimator.ModeKeys.EVAL, tf.estimator.ModeKeys.PREDICT):
            detections = detection_model.postprocess(
                prediction_dict,
                features[fields.InputDataFields.true_image_shape])

        # 3.load var value from ckpt --放在梯度计算前
        if mode == tf.estimator.ModeKeys.TRAIN:
            if train_config.fine_tune_checkpoint and hparams.load_pretrained:
                # 这是一个兼容语句，fine_tune_checkpoint_type以后是必须给出的参数
                if not train_config.fine_tune_checkpoint_type:
                    # train_config.from_detection_checkpoint field is deprecated. For
                    # backward compatibility, set train_config.fine_tune_checkpoint_type
                    # based on train_config.from_detection_checkpoint.
                    if train_config.from_detection_checkpoint:
                        train_config.fine_tune_checkpoint_type = 'detection'  # 整个模型参数
                    else:
                        train_config.fine_tune_checkpoint_type = 'classification'  # 基础网络参数
                # 想要获得的参数
...
def create_estimator_and_inputs(run_config,
    ...
    # 配置参数准备
    # 0.1 通过pipeline config文件解析得到一个基础的config字典
    configs = config_util.get_configs_from_pipeline_file(pipeline_config_path, config_override=config_override)
    # 0.2
    kwargs.update({
    ...
    # 0.3 更新config字典
    configs = config_util.merge_external_params_with_configs(configs, hparams, kwargs_dict=kwargs)
    # 0.4 单独提取出各部分参数
    ...
    # 4.定义estimator
    model_fn = model_fn_creator(model_builder.build, configs, hparams)
    # 解释一下model_fn的两个作用，1.构建计算图 2.用计算图中的重要节点 初始化一个tf.estimator.EstimatorSpec对象，
    estimator = tf.estimator.Estimator(model_fn=model_fn, config=run_config)

    # 5.保存一份pipeline config在model_dir
...
def create_train_and_eval_specs(train_input_fn,
    ...
    Args:
      ...
      eval_input_fn: Function that produce features and labels on eval data.
    ...
    eval_spec = tf.estimator.EvalSpec(
        ...
        throttle_secs=600)  # 写出来了全部参数，供学习 参考 throttle_sec间隔600s评估一次
