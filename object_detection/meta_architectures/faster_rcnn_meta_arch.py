...
class FasterRCNNMetaArch(model.DetectionModel):
    ...
    def predict(self, preprocessed_inputs):
        ...
        """
        # 1.得到 两个feature map/box_list类的anchor/resize后的image_shape
        (rpn_box_predictor_features, rpn_features_to_crop, anchors_boxlist,
         image_shape) = self._extract_rpn_feature_maps(preprocessed_inputs)

        # 2.对RPN产生的feature map分别用1*1的卷积核 得到anchor的位置偏移量和正负类别
        (rpn_box_encodings, rpn_objectness_predictions_with_background
         ) = self._predict_rpn_proposals(rpn_box_predictor_features)

        # 3.anchor修正与筛选
        # The Faster R-CNN paper recommends
        # pruning anchors that venture outside the image window at training time and
    ...
    def _predict_second_stage(self, rpn_box_encodings,
        ...
        Returns:
          prediction_dict: a dictionary holding "raw" prediction tensors:
            ...
            7) mask_predictions: (optional) a 4-D tensor with shape
              ...
              containing instance mask predictions.
        """
        # 1.对PRN网络产生的候选框进行处理-NMS,正负样本采样以及坐标归一化,num_proposals是采样之后真实的数量
        ...
        # 4.预测box位置偏移量和类别 --core->box_predictor
        # 全连接层运算
        ...
        if box_predictor.MASK_PREDICTIONS in box_predictions:
            mask_predictions = tf.squeeze(box_predictions[box_predictor.MASK_PREDICTIONS], axis=1)
    ...
    def _compute_second_stage_input_feature_maps(self, features_to_crop, proposal_boxes_normalized):
        ...
        def get_box_inds(proposals):
            ...
            # value[0,0,0...0,1,1..1,....batch_size,batch_size...batch_size]
            return tf.reshape(ones_mat * multiplier, [-1])  # shape=[-1]时， flattens into 1-D.

        # 裁剪出k个子区域并resize到目标形状(14,14),最终shape(batch*num_proposal,14,14,3)
        cropped_regions = tf.image.crop_and_resize(
            features_to_crop,
            self._flatten_first_two_dimensions(proposal_boxes_normalized),  # 将前两维[batch, num_anchor]展开
            get_box_inds(proposal_boxes_normalized),  # 候选框所在的batch
    ...
    def postprocess(self, prediction_dict):
        ...
        """
        # 对RPN预测结果按照配置参数进行NMS,并将候选框坐标归一化
        # 训练阶段,则需要按照第二阶段数量阈值限制,对候选框均衡采样
    ...
    def _postprocess_rpn(self,
        ...
        """
        # 1.数据预处理
        # 1.1 处理box_encoding, 得到预测的bbnd_box
        # 增加大小为1的num_class维度,[batch_size, num_anchors, 1, self._box_coder.code_size]
        # 为了满足函数要求，经函数处理后再删掉
        rpn_box_encodings_batch = tf.expand_dims(rpn_box_encodings_batch, axis=2)
        # 得到新的shape--有时tensor的维度可以设为None,故使用此方法
        rpn_encodings_shape = shape_utils.combined_static_and_dynamic_shape(rpn_box_encodings_batch)
        # 将anchor增加一个batch维度，[1, num_anchors, 4]->[batch_size, num_anchor, 4]
        ...
        # 1.2 处理prediction_class, 对axis=-1进行概率归一化--softmax, 只保留前景(目标)anchor
        # [batch_size, num_anchors]
        rpn_objectness_softmax_without_background = tf.nn.softmax(
            rpn_objectness_predictions_with_background_batch)[:, :, 1]
        # 1.3 图片大小 tf.stack()将内容合并 结果是 增加一个维度，维度大小为元素个数
        clip_window = tf.to_float(tf.stack([0, 0, image_shape[1], image_shape[2]]))

        # 2.非极大值抑制
        # 返回值num_proposal.shape = [batch],一个batch内每张图片的候选框数量
        # 搞明白原理
        ...
        # 3.训练阶段，产生采用均衡采样的候选框
        # 以及每张图片候选框数量, 最大值从_first_stage_max_proposals(config)——>_second_stage_batch_size(64)
        if self._is_training:
            # 在此节点的梯度停止传播, 搞明白
    ...
    def _unpad_proposals_and_sample_box_classifier_batch(
        ...
        for (single_image_proposal_boxes,
            ...
            single_image_proposal_scores = tf.slice(single_image_proposal_scores,
                                                    [0],
                                                    [single_image_num_proposals])
            # step2 将推荐框bbox score生成一个BoxList类
    ...
    def _postprocess_box_classifier(self,
        ...
        # 5.NMS
        # 还是在调用post_processing.batch_multiclass_non_max_suppression
        # 使用偏函数将score, iou, per_class_max, total_max定义
    ...
    def loss(self, prediction_dict, scope=None):
        ...
        with tf.name_scope(scope, 'Loss', prediction_dict.values()):
            (groundtruth_boxlists, groundtruth_classes_with_background_list,
             groundtruth_masks_list) = self._format_groundtruth_data(prediction_dict['image_shape'])
            # 第一阶段loss
    ...
    def _loss_rpn(self,
        ...
        with tf.name_scope('RPNLoss'):
            # step1 根据标注为每个anchor分配所属类别和偏移量，即整个模型的学习目标
    ...
    def _loss_box_classifier(self,
        ...
        with tf.name_scope('BoxClassifierLoss'):
            ...
            num_proposals_or_one = tf.to_float(tf.expand_dims(tf.maximum(num_proposals, tf.ones_like(num_proposals)),
                                                              1))
            ...
            # reshape
            # 第二阶段的预测 以所有的推荐框作为new_batch, 在此处重新按照图片数量组成batch
