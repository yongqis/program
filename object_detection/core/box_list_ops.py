...
def prune_outside_window(boxlist, window, scope=None):
    ...
    window.
    See also clip_to_window which only prunes bounding boxes that fall
...
def iou(boxlist1, boxlist2, scope=None):
    ...
    with tf.name_scope(scope, 'IOU'):
        # intersections是一个shape[N,M]的tensor
        intersections = intersection(boxlist1, boxlist2)
        # area计算box面积，返回一个shape[num_box]的tensor
        areas1 = area(boxlist1)
        areas2 = area(boxlist2)
        # [N,1]+[1,M]-[N,M]
        unions = (tf.expand_dims(areas1, 1) + tf.expand_dims(areas2, 0) - intersections)
        # tf.equal()返回一个shape[N,M] bool类型的tensor
        # True时选择tf.zeros_like对应位置的值
        # False时，选择tf.truediv对应位置的值
        return tf.where(
            tf.equal(intersections, 0.0),
            tf.zeros_like(intersections),  # 返回一个形状和类型相同的0值tensor
            tf.truediv(intersections, unions)  # 对应位置相除
        )
