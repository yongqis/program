...
def multiclass_non_max_suppression(boxes,
    ...
    with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
        ...
        selected_boxes_list = []  # 保存满足阈值要求的BoxList类box
        # 1.拆分类别[k,q,4]->q个[k,4] 组成list,q是类别数量，即首先进行统一类别内的box筛选
...
def batch_multiclass_non_max_suppression(boxes,
    ...
    with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
        ...
        def _single_image_nms_fn(args):
            ...
            # 4.如果不足max_total_size个box, 对应位置填充0
            padded_boxlist = box_list_ops.pad_or_clip_box_list(nmsed_boxlist, max_total_size)
            # 5.分别获取对应信息
            num_detections = nmsed_boxlist.num_boxes()  # 注意来源，记录的是真实数量
        ...
        # tf.map_fn()依次将单张图片信息/elems内元素的第一维顺序，送入函数
        # 得到batch个图像的候选框
