...
class WeightedSmoothL1LocalizationLoss(Loss):
    ...
    def _compute_loss(self, prediction_tensor, target_tensor, weights):
        ...
        Returns:
          loss: a (scalar) tensor representing the value of the loss function
        """
        # 首先求四个点的 预测偏移量 与 目标偏移量 之间的差值
        diff = prediction_tensor - target_tensor
        # 取绝对值
        abs_diff = tf.abs(diff)
        # 将结果分为 绝对值小于1和大于等于1的分段函数，
        # 对第3维的loss求和，得到一个预测框的总损失，shape[batch,num_anchor],最后乘以weights表示去掉负类的loss
        abs_diff_lt_1 = tf.less(abs_diff, 1)
        anchorwise_smooth_l1norm = tf.reduce_sum(tf.where(abs_diff_lt_1, 0.5 * tf.square(abs_diff), abs_diff - 0.5),
                                                 2) * weights
        if self._anchorwise_output:
