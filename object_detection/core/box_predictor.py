...
class MaskRCNNBoxPredictor(BoxPredictor):
    ...
    def _predict(self, image_features, num_predictions_per_location):
        ...
        spatial_averaged_image_features = tf.reduce_mean(image_features, [1, 2],
                                                         keepdims=True,
                                                         name='AvgPool')
        # 将图像展开,保留batch。故shape为[batch,channels]，batch为所有图片的推荐框的总数
        ...
        with slim.arg_scope(self._fc_hyperparams):
            # 位置预测结果[batch, num_class*4]
            box_embeddings = slim.fully_connected(
                flattened_image_features,
                512,
                activation_fn=None,
                scope='BoxEmbeddings')

        box_embeddings = tf.nn.l2_normalize(box_embeddings, 1, 1e-10, name='embeddings')
        # predictions_dict = {'box_embeddings': box_embeddings}
...
class ConvolutionalBoxPredictor(BoxPredictor):
    ...
    def _predict(self, image_features, num_predictions_per_location):
        ...
        net = image_features
        with slim.arg_scope(self._conv_hyperparams), slim.arg_scope([slim.dropout], is_training=self._is_training):
            ...
            # 直接看这里，slim.arg_scope(设置一些操作的默认值)
...
class RegionPredictor(BoxPredictor):
    def __init__(self,
                 is_training,
                 num_classes,
                 fc_hyperparams,
        ...
        Raises:
          ValueError: If predict_instance_masks or predict_keypoints is true.
        """
        super(RegionPredictor, self).__init__(is_training, num_classes)
        self._fc_hyperparams = fc_hyperparams
        self._use_dropout = use_dropout
        self._box_code_size = box_code_size
        self._dropout_keep_prob = dropout_keep_prob
