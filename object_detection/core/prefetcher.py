...
def prefetch(tensor_dict, capacity):
    ...
    # 1.创建一个支持padding的FIFO队列，A FIFOQueue that supports batching variable-sized tensors by padding.
    ...
    # 3.队列启动器加入集合
    # 为队列创建启动器：tf.train.QueueRunner(queue, [enqueue_op])
    # 将启动器加入默认集合：add_queue_runner: Adds a `QueueRunner` to a collection in the graph.
    # 启动集合里的队列：tf.train.start_queue_runners(sess=sess, coord=coord)
    tf.train.add_queue_runner(tf.train.QueueRunner(prefetch_queue, [enqueue_op]))
