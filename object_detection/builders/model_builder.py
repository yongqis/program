...
from object_detection.meta_architectures import faster_rcnn_joint_triplet_loss_meta_arch
...
def build(model_config, is_training, is_building=False):
    ...
    if meta_architecture == 'faster_rcnn':
        return _build_faster_rcnn_joint_triplet_loss_model(model_config.faster_rcnn, is_training, is_building)
...
def _build_faster_rcnn_model(frcnn_config, is_training):
    ...
    Raises:
      ValueError: If frcnn_config.type is not recognized (i.e. not registered in
        model_class_map).
    """
    # 所有参数的默认值在protos文件中，config文件给的是可以修改的，不会破坏模型完整的参数，没有给的是保证模型完整性
...
def _build_faster_rcnn_joint_triplet_loss_model(frcnn_config, is_training, is_building=False):
    """Builds a Faster R-CNN or R-FCN detection model based on the model config.
    ...
    if isinstance(second_stage_box_predictor, box_predictor.RfcnBoxPredictor):
        ...
        return faster_rcnn_joint_triplet_loss_meta_arch.FasterRCNNMetaArch(
            initial_crop_size=initial_crop_size,
            maxpool_kernel_size=maxpool_kernel_size,
            maxpool_stride=maxpool_stride,
            second_stage_mask_rcnn_box_predictor=second_stage_box_predictor,
