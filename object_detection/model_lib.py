...
def create_model_fn(detection_model_fn, configs, hparams, use_tpu=False, postprocess_on_cpu=False):
    ...
    def model_fn(features, labels, mode, params=None):
        ...
        if mode in (tf.estimator.ModeKeys.EVAL, tf.estimator.ModeKeys.PREDICT):
            if use_tpu and postprocess_on_cpu:
                detections = tf.contrib.tpu.outside_compilation(
                    postprocess_wrapper, (prediction_dict, features[fields.InputDataFields.true_image_shape]))
            else:
                detections = postprocess_wrapper((
                    prediction_dict,
                    features[fields.InputDataFields.true_image_shape]))

        # load var value from ckpt
        if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        # total loss and build optimizer--为什么eval也需要loss和optimizer?
        if mode in (tf.estimator.ModeKeys.TRAIN, tf.estimator.ModeKeys.EVAL):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            if use_tpu:
                training_optimizer = tf.contrib.tpu.CrossShardOptimizer(training_optimizer)

            # 梯度冻结-Optionally freeze some layers by setting their gradients to be zero.
            trainable_variables = None
            include_variables = (
                train_config.update_trainable_variables if train_config.update_trainable_variables else None)
            exclude_variables = (
...
def create_estimator_and_inputs(run_config,
    ...
    # 0.2 配置参数准备
    # 0.2.1 pipeline config文件解析得到一个基础的config字典
    ...
    eval_on_train_input_config = copy.deepcopy(train_input_config)   # 使用训练数据来评估
    ...
    if override_eval_num_epochs and eval_on_train_input_config.num_epochs != 1:
        tf.logging.warning('Expected number of evaluation epochs is 1, but '
                           ...
                           '.num_epochs` = '
                           '{}. Overwriting `num_epochs` to 1.'.format(eval_on_train_input_config.num_epochs))
    ...
    detection_model_fn = functools.partial(detection_model_fn_base, model_config=model_config)
    ...
    # Write the as-run pipeline config to disk. 保存一份config在model_dir
...
def create_train_and_eval_specs(train_input_fn,
    ...
    for index, (eval_spec_name, eval_input_fn) in enumerate(zip(eval_spec_names, eval_input_fns)):
        ...
        # FinalExporter导出最后时评估的模型计算图 BestExporter导出最佳评估结果的计算图 LatestExporter持续导出
